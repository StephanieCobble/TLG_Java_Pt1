...
//        writePoem();
    ...
    private static void readPoem() {
        try {
            String poem = Files.readString(Paths.get("famous-poem.txt")); // Java 11 would be Path.of()
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader("famous-poem.txt"))) {
            String line;
            while ((line = reader.readLine()) != null) { // null means EOF; (ref pg 352 main)
    ...
    private static void writePoem() {
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter("haiku.txt"))) {
            writer.println("Halloween is great");
            writer.println("It's the best time of the year");
            writer.println("Time to trick or treat");
        }
        catch (IOException e) {
            e.printStackTrace();
        }
